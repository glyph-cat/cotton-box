...
We can create a watch handler to observe state changes with the `.watch` method.

It accepts a callback function as its parameter and it will be invoked whenever the state changes.

The `.watch` method returns a callback, that when called, will remove the watch handler.
