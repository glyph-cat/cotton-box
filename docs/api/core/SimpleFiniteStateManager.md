...
- can stand be standing `idle`, `walking`, or `attacking`;
- can only attack only when standing still.
...
Since the initial state is `'idle'`, changing the state to `'walking'` immediately after instantiating the class would be just fine.
```ts
