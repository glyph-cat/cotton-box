...
describe(Watcher.name, () => {
  ...
  test('Unwatch All & Dispose', () => {
    ...
    const stopWatching2 = watcher.M$watch((num: number) => { counter2 += num })
    ...
    const unwatch3 = watcher.M$watch((num: number) => { counter3 += num })
    ...
    const unwatch4 = watcher.M$watch((num: number) => { counter4 += num })
