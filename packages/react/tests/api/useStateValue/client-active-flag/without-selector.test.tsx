  ...
  describe(StateManagerTypeKey, () => {
    ...
    describe('Only test initial value', () => {
      ...
      test('active=true', () => {
        const TestState = new StateManagerType(42)
        collectForTeardown(TestState.dispose)
        const hook = customRenderHook(() => useStateValue(TestState, null, true))
        const { result } = hook
        collectForTeardown(hook.unmount)
        expect(result.current).toBe(42)
      })
      ...
      test('active=false', () => {
        const TestState = new StateManagerType(42)
        collectForTeardown(TestState.dispose)
        const hook = customRenderHook(() => useStateValue(TestState, null, false))
        const { result } = hook
        collectForTeardown(hook.unmount)
        expect(result.current).toBe(42)
      })
    ...
    describe('Test state changes', () => {
      ...
      test('active = true -> false -> true', async () => {
        ...
        const hook = customRenderHook<HookProps, number>(({ active }) => {
          return useStateValue(TestState, null, active)
        }, {
          initialProps: {
            active: true,
          },
