...
export const TSDOC_DESC_INVALID_STATE_TRANSITION_ERROR = `An error thrown by [\`SimpleFiniteStateManager\`](${DOCS_API_CORE_URL}/SimpleFiniteStateManager) when attempting to change a state without conforming to the \`allowedStateTransitions\` in the constructor. You may create instances of this error or even extend it, but there is almost never a scenario in which this is necessary.`
...
export const TSDOC_METHOD_DESC_TRY_RESET = 'Tries to reset the State Manager back to it\'s default value. If the reset does not conform to the defined state transitions, the state will not change and no error will be thrown.'
...
export const RETURN_DESC_TRY_RESET = '`true` if the state reset conforms to the predefined state transitions, otherwise `false`.'
