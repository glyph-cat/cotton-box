...
{/* Construction formula for this article: Copy everything to SimpleStateManager, change signature, change `options` param for constructor, then add a section for `init` and mention which properties and methods are inherited */}
...
<MarkdownWrapper>{DocConstants.TSDOC_METHOD_DESC_WAIT_FOR_INIT}</MarkdownWrapper>

```ts
waitForInit(state?: boolean): Promise<void>
```
...
<ObjectShapeDefinition
  data={[{
    ...
    description: DocConstants.TSDOC_PARAM_DESC_WAIT_FOR_INIT_STATE,
    isOptional: true,
  }]}
/>

#### Returns
<Returns name='waitForInit'>{DocConstants.RETURN_DESC_WAIT_FOR_INIT}</Returns>

#### Example
```ts
await ExampleState.waitForInit()
```
